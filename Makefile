...
run-tests: $(TESTS) ## Run all CI tests under test/ci
	 bash -c "diff <(gnmic --address clab-acl-git-srl --username admin --password NokiaSrl1\! get --path '/acl' --skip-verify -e JSON_IETF | jq '.updates[0].values.[\"srl_nokia-acl:acl\"]') ./srl.srl_acl" || true
